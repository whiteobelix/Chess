...
public class BoardManager {
	...
	/**
	 * Metoda oblicza czy dany ruch dan¹ bierk¹ mo¿e byæ wykonany i zwraca typ
	 * Move (koordynaty, rodzaj ruchu i rodzaj bierki).
	 */
	...
	Coordinate kingSearching(Color kingColor) {
		...
		for (int x = 0; x < Board.SIZE - 1; x++) {
			for (int y = 0; y < Board.SIZE - 1; y++) {
				if (this.board.getPieceAt(x, y) != null && this.board.getPieceAt(x, y).getType() == PieceType.KING
						&& this.board.getPieceAt(x, y).getColor() == kingColor) {
	...
	boolean isKingInCheckByKnight(Color kingColor, int kingX, int kingY) {
		...
		int[][] knightFields = { { kingX + 1, kingY + 2 }, { kingX + 1, kingY - 2 }, { kingX + 2, kingY + 1 },
				{ kingX + 2, kingY - 1 }, { kingX - 1, kingY + 2 }, { kingX - 1, kingY - 2 }, { kingX - 2, kingY + 1 },
				{ kingX - 2, kingY - 1 } };
		for (int knightField[] : knightFields) {
			Coordinate potentialKnightCoordinate = new Coordinate(knightField[0], knightField[1]);
			if (this.board.getPieceAt(kingX, kingY) != null && knightField[0] >= 0 && knightField[0] <= Board.SIZE
					&& knightField[1] >= 0 && knightField[1] <= Board.SIZE
					&& this.board.getPieceAt(potentialKnightCoordinate) != null
					&& this.board.getPieceAt(potentialKnightCoordinate).getType() == PieceType.KNIGHT
	...
	boolean isKingInCheckByOtherFigureThanKnight(Color kingColor, int kingX, int kingY)
	...
	boolean isKingInCheck(Color kingColor) throws InvalidMoveException { 

		boolean isKingInCheck = false;
		int kingX = kingSearching(kingColor).getX();
		int kingY = kingSearching(kingColor).getY();

		if (isKingInCheckByKnight(kingColor, kingX, kingY) == true
				|| isKingInCheckByOtherFigureThanKnight(kingColor, kingX, kingY) == true) {
			isKingInCheck = true;
