...
//TODO tu s¹ metody wykorzystuj¹ce isKingInCheck
	...
	private Move validateMove(Coordinate from, Coordinate to) throws InvalidMoveException, KingInCheckException {
		// (8 x 2 figur ka¿dego koloru jest razem z pionkami)

		MoveValidatingForEachPiece oneMove = new MoveValidatingForEachPiece();
	...
	Coordinate kingSearching(Color kingColor){
		...
		for(int x = 0; x < Board.SIZE - 1; x++){
			for(int y = 0; y < Board.SIZE - 1; y++){	
				if (this.board.getPieceAt(x, y).getType() == PieceType.KING && this.board.getPieceAt(x, y).getColor() == kingColor){
	...
	boolean isKingInCheckByKnight(Color kingColor, int kingX, int kingY){
		...
		for(int knightField[] : knightFields){
			...
			if (knightField[0] >= 0 && knightField[0] <= Board.SIZE && knightField[1] >= 0 && knightField[1] <= Board.SIZE && this.board.getPieceAt(potentialKnightCoordinate).getType() == PieceType.KNIGHT && this.board.getPieceAt(potentialKnightCoordinate).getColor() != kingColor){
						...
						if (iAroundX != 0){
							for(int iAroundY = -1; iAroundY < 2; iAroundY++){
								if (iAroundY != 0){
									...
									validateMove(fromCoord, toCoord); // zwraca move, który zawiera koordynaty from i to, rodzaj ruchu oraz poruszan¹ bierkê.
//									if (){
//										
//									}
								}
