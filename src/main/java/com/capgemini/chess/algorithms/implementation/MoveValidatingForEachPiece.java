...
public class MoveValidatingForEachPiece extends BoardManager{
	...
	MoveValidatingForEachPiece(Board board) {
		this.board = board;
	}
...
//		boolean	b3333 = deltaX(from,to) == 0;
		...
		// boolean mówi¹cy wprost, czy pole "to" jest zajête: board.getPieceAt(from).equals(board.getPieceAt(to));
			...
			if(isThisMoveAlloved == true){
				...
				thisMove = someMove;
			}else throw new InvalidMoveException("Nie mozna wykonaæ ruchu.");

		
		if(thisMove == null) throw new InvalidMoveException("Pusty obiekt ruchu.");
		return thisMove;
	}
...
//			}
	...
	boolean queenValidating(Coordinate from, Coordinate to) throws InvalidMoveException {//ruchy w ka¿d¹ stronê
		if(Math.abs(deltaX(from,to)) == Math.abs(deltaY(from,to)) && isItPathFree(from, to) == true && Math.abs(deltaX(from,to)) != 0){//czy figura to królowa && czy porusza sie po skosie && czy miejsce jest zajête && czy droga wolna
			isMoveAllowed = true;
...
//		int g = 3;
	...
	Piece getMyPiece(Coordinate point) throws InvalidMoveException{
